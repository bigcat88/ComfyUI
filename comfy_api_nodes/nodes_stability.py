...
class StabilityTextToAudio(comfy_io.ComfyNode):
    ...
    def define_schema(cls):
        return comfy_io.Schema(
            ...
            display_name="Stability Text To Audio",
    ...
    async def execute(cls, model: str, prompt: str, duration: int, seed: int, steps: int) -> comfy_io.NodeOutput:
        validate_string(prompt, min_length=1, max_length=9999)
...
class StabilityAudioToAudio(comfy_io.ComfyNode):
    ...
    def define_schema(cls):
        return comfy_io.Schema(
            ...
            display_name="Stability Audio To Audio",
            category="api node/audio/Stability AI",
            description=cleandoc(cls.__doc__ or ""),
            inputs=[
                comfy_io.Combo.Input(
    ...
    @classmethod
    async def execute(
        cls, model: str, prompt: str, audio: AudioInput, duration: int, seed: int, steps: int, strength: float
    ) -> comfy_io.NodeOutput:
        validate_string(prompt, min_length=1, max_length=9999)
...
class StabilityAudioInpaint(comfy_io.ComfyNode):
    ...
    def define_schema(cls):
        return comfy_io.Schema(
            ...
            display_name="Stability Audio Inpaint",
            category="api node/audio/Stability AI",
            description=cleandoc(cls.__doc__ or ""),
            inputs=[
                comfy_io.Combo.Input(
    ...
    async def execute(
        ...
        operation = SynchronousOperation(
            endpoint=ApiEndpoint(
                path="/proxy/stable-audio/v2beta/audio/stable-audio-2/audio-to-audio",
                method=HttpMethod.POST,
